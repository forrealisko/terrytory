...
if (flag === "--stats") {
  ...
  console.log("\n╔══════════════════════════════════════╗");
  console.log("║  THE DEBRIEF — SCRAPER STATS         ║");
  console.log("╚══════════════════════════════════════╝\n");
...
if (flag === "--all") {
  ...
  latest.headlines.forEach((h) => {
    ...
    console.log(`${flag} [${h.type.padEnd(11)}] [${age.padEnd(10)}] ${h.title}${cats}`);
    if (h.author) console.log(`     ✍️  ${h.author}`);
  });
  console.log();
}
...
if (flag === "--new") {
  ...
  newOnes.forEach((h) => {
    ...
    console.log(`    Author:     ${h.author || "unknown"}`);
    console.log(`    Date:       ${h.article_date || "unknown"} (${h.date_analysis?.relative || "?"})`);
    if (h.categories?.length) console.log(`    Categories: ${h.categories.join(", ")}`);
